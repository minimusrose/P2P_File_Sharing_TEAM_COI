...
pm.handle_peer_announce('local_peer', '192.168.1.5', 5001)  # Devrait etre ignore

peers = pm.get_online_peers()
print(f"OK Peers en ligne: {len(peers)} (devrait etre 2)")
for p in peers:
    print(f"  - {p['peer_id']}")
print()
...
print(f"OK Fichiers ajoutes: {len(all_files)}")
for f in all_files:
